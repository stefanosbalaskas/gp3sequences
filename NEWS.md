...
* Registers the September 2026 ecosystem-methods article in the pkgdown article index so the complete documentation site builds successfully.
* Adds exact-tag R CMD check triggering for immutable `v*` release tags and extends the matrix with Ubuntu oldrel-1.
* Preserves the complete 0.3.0 scientific API, defaults, estimands, analysis contracts, modelling semantics, and interpretation boundaries.
